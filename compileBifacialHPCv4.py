...
resultsfolder = '/home/akinzer/JordanWorld/'
...
locations_co = [l.replace('.', '__') for l in locations_co] # replace periods with double underscore for consistency
...
errors_all = []
for ii in range(0, len(locations_co)):
    location = locations_co[ii][:-5] # drops "__epw" at end of location name
...
df.to_csv(os.path.join(savefolder,'Results_CO.csv'))
